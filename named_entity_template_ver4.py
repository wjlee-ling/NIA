...
template_movie = ['특선 영화로 {}를 방영한대.', '내일 {}가 재개봉해요.', '난 {}는 생각보다 별로였어.', '영화 {} 정보 검색해줘.', '어제 {} 시사회 다녀왔어.',
                  '그 배우가 {}에 나와?', '그 감독이 {}도 만들었어요?', '내 친구 {} 촬영 스태프였어.', '재개봉하면 꼭 {} 보러 가자.']
template_tvseries = ['월요일엔 꼭 {} 챙겨 봐야 해.', '어제 {} 재밌었어요?', '컴퓨터로 {} 다시 보자.', '요즘 {}만 챙겨 봐요.',
                     '그 연예인이 {}에도 나왔어요?', '시청률은 {}가 높았어요.', '공부하느라 {}를 못 봤네.', '운동하면서 {} 한 편을 다 봤어.']
...
template_company = ['가장 좋아하는 브랜드/카페/호텔/어쩌구는 {}예요.', '나랑 {}에서 알바/인턴 하자.', '내일 {} 할인해요.', '어제 {} 채용 공고 올라온 거 봤어?',
                    '곧 {} 주가가 오를 것 같아요.', '여기 {} 매장 있어요?', '요즘 {}도 복지가 좋대요.', '이따가 {} 본사도 한번 가 보자.', '월요일부터 {}로 출근해요.']
...
def write_script(df, name):
    ...
    return df.to_excel(f'외래어_스크립트_양식(최종)_jspark_개체명_Set4_{name}.xlsx')
...
def check_script(df):
    for row in range(len(df['지문'])):
        entity = re.match('[\w ]+', df['상황'][row]).group().strip()
        sent =

# Download the module 'openpyxl' before running the following codes
df = pd.read_excel(r'C:\Users\oian\Documents\NIA\외래어_개체명_Set4\외래어_스크립트_양식(최종)_jspark_이원재.xlsx',sheet_name= '개체명 외래어',  header=0)
write_script(df, '이원재')
