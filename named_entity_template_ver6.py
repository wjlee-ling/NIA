...
def check_env(sent, entity):
    sent= re.sub('\s+', '', sent)
    if re.search('r[0-9]', entity):
        print(entity+'에는 숫자가 포함되어 있음. 지문에는 문자열로 썼는지 확인 바람.')
    else:
        try:
            prev_chr = kor_chr(re.search('\w(?=' + entity + ')', sent).group())
        except AttributeError:
            print(entity+'앞 음절 관련 에러')
        try:
            fol_chr = kor_chr(re.search('(?<='+entity+')\w', sent).group())
        except AttributeError:
            print(entity + '뒤 음절 관련 에러')
        jung, jong = [], []  # lists of unicode values of the previous/following characters
        if prev_chr[2] == 0: # 앞 음절에 종성이 없으면
            if prev_chr[1] in [1, 5]: # 앞 음절의 종성이 'ㅐ' 이거나 'ㅔ'이면
                jung.append(1) # 'ㅐ' 추가
            else:
                jung.append(prev_chr[1]) # 앞 음절의 중성 추가
        elif prev_chr[2] in [1, 2, 3, 9, 24]: # 종성이 'ㄱ', 'ㄲ', 'ㄳ', 'ㄺ', 'ㅋ'
            jong.append(1)
        elif prev_chr[2] in [7, 19, 20, 22, 23, 25]: # 종성이 'ㄷ', 'ㅅ', 'ㅈ', 'ㅆ', 'ㅊ', 'ㅌ'
            jong.append(7)
            ...
            jong.append(16)
        elif prev_chr[2] == 21: #ㅇ  cf. 종성'ㅎ'(27)는 개체명 앞에 나올 수 없으므로 무시
            jong.append(21)

        if len(jong) + len(jung) != len(set(jong)) + len(set(jung)):
            print(entity,'앞 음절에 중복 있음!')

        jung, cho = [], []
        if fol_chr[0] == 11: # 다음 음절 초성이 'ㅇ'이면
            if fol_chr[1] in [1, 5]: # ㅐ/ㅔ
                jung.append(fol_chr[1]) # ㅐ 추가
            else:
                jung.append(fol_chr[1])
        else:
            cho.append(fol_chr[0])
        if len(jung) + len(cho) != len(set(jung)) + len(set(cho)):
